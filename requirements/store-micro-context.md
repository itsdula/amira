...
| **`inbound-brain` (Edge Function)** | ask_channel miss path + every gather turn | The model turn: pack + inbound text → LLM behind a JSON schema → `{reply, actions[]}`. Executor validates each action against the pack (step fact allowlists, legal transitions, catalogue-grounded values, channel-only-when-unset, opt-out add-only), writes accepted state **before** returning the reply, drops+logs rejections (audit row in `messages.meta.kind=brain_audit` with `model_ms`/`total_ms`). Reply discarded for a safe canned line when a rejected action would make the words lie about state. Auth: service-role key header. Model unconfigured → deterministic fallback question, flow keeps working. |
