...
| Re-asked a fact the customer already gave or declined | `brain_audit` row first: was `upsert_fact` rejected? Then `_shared/brain-contract.ts` allowlists vs `[CONTEXT]` covered-facts line | A stored fact re-asked is a defect by spec. |
...
| No reply at all on WhatsApp | AF flow runs panel (which node failed), channel `webhookUrl`, then `store_inbound` HTTP status | Flow import/wiring issues are covered in `flows/RECIPES.md` debugging notes. |
