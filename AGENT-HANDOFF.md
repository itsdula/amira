...
- **Dialer** lives in the brain executor (v11): `set_channel_choice` returns `dial_now:true` (call_now, in hours) → `POST /call` with lead pack as `variables`, `lead_id` in call `metadata`. Phone number id auto-discovered from `GET /phone-number` (first entry, cached).
