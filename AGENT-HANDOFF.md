...
**Pending:** `supabase/migrations/20260914160000_channel_choice_outbound.sql` (`set_channel_choice` + `record_outbound`) is written but **not applied** — apply before first inbound-flow run.

## Flows (repo `flows/`)

`flows/amira-inbound-whatsapp.json` — generated by `flows/build-inbound-flow.mjs` (rerun after edits; do not hand-edit). Wiring, node recipes, and test curls: `flows/RECIPES.md`. Paste-ready test payloads: `flows/samples/`. Needs AF variables `SUPABASE_SERVICE_ROLE_KEY` + `AMIRA_CHAT_ASSISTANT_ID` and the channel `webhookUrl` pointed at the flow. The old `ak_71a45b87…` key was scrubbed from `whatsapp-messaging.json` — rotate it if it is still the live workspace key.
...
1. Apply the pending migration, import `flows/amira-inbound-whatsapp.json`, set variables, wire channel `webhookUrl` (see `flows/RECIPES.md`).
